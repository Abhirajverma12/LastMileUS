  ...
  return (
    <div className="landing-layout">
      ...
      <nav className="landing-navbar">
        <div className="landing-logo" style={{ color: '#eab308', textShadow: 'none', gap: '0.2rem' }}>
          <span style={{ color: '#eab308' }}>LastMile</span><span style={{ color: '#fff' }}>US</span>
        </div>
        <div className="landing-nav-links" style={{ gap: '2rem' }}>
          {user ? (
            <>
              <span style={{ color: '#a1a1aa', fontSize: '0.9rem' }}>Welcome, {user.name}</span>
              <Link to={getDashboardLink()} className="btn" style={{ background: '#eab308', color: '#000', borderRadius: '4px', padding: '0.5rem 1.25rem', fontWeight: 700 }}>Go to Dashboard</Link>
              <button onClick={handleLogout} className="btn btn-outline" style={{ border: '1px solid #333', color: '#fff', borderRadius: '4px', padding: '0.5rem 1.25rem' }}>Sign Out</button>
            </>
          ) : (
            <>
              <Link to="/login" className="btn" style={{ background: '#eab308', color: '#000', borderRadius: '4px', padding: '0.5rem 1.25rem', fontWeight: 700 }}>Sign In</Link>
              <Link to="/register" className="btn btn-outline" style={{ border: '1px solid #333', color: '#fff', borderRadius: '4px', padding: '0.5rem 1.25rem' }}>Create Account</Link>
            </>
          )}
        </div>
      </nav>
      ...
      <div className="landing-content">
        ...
        <div className="landing-top-section">
          <div className="landing-text-col">
            <div className="badge-outline" style={{ color: '#eab308', borderColor: 'rgba(234, 179, 8, 0.3)', marginBottom: '1.5rem', display: 'inline-block', padding: '0.25rem 1rem', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 'bold', letterSpacing: '1px' }}>
              SMART LOGISTICS
            </div>
            ...
            <div style={{ marginTop: '2rem' }}>
               <Link to={user ? getDashboardLink() : '/login'} className="btn" style={{ background: '#eab308', color: '#000', borderRadius: '8px', padding: '0.85rem 2rem', fontSize: '1rem', fontWeight: 700 }}>
